...
type ordAcct struct {
	...
	// deferred (makers only): this order's retirement could not be confirmed and now lives in
	// Engine.retireQ — retireOrder short-circuits so the obligation loop is the only path that
	// keeps asking the broker about it.
	deferred bool
}
