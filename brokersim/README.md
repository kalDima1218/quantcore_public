...
Харнесс работает через узкий тестовый `harnessEngine`; адаптеры для `execengine`
и `execengine2` скрывают различия сигнатур событий и конструкторов. Поэтому один
сценарий принимает фабрику движка и не меняется при переключении версии;
`TestEngineHappyPath` уже запускается для обеих реализаций. Расширенный набор
исторических fault-сценариев по умолчанию остаётся на `execengine`.
